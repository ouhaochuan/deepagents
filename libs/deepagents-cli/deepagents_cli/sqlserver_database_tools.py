...
def create_database_tools(connection_string):
    """
    创建数据库工具
    ...
    # 只创建3个基本工具，无LLM依赖
    tools = [
        InfoSQLDatabaseTool(db=db, name="sqlserver_info_sql_database_tool"),      # 列出表
        ListSQLDatabaseTool(db=db, name="sqlserver_list_sql_database_tool"),      # 获取schema  
        QuerySQLDatabaseTool(db=db, name="sqlserver_query_sql_database_tool")     # 执行查询
    ]
...
def test_database_tools():
    ...
    test_connection_string = "mssql+pyodbc://prg:prg@10.196.85.17\\sql2012/ZY_Pgplatform?driver=ODBC+Driver+17+for+SQL+Server"
    
    try:
        tools = create_database_tools(test_connection_string)
        ...
        print("\n--- 测试执行查询 ---")
        try:
            sample_query = "SELECT TOP 5 * FROM " + (tables.split(',')[0].strip() if tables and ',' in tables else tables.strip() if tables else "your_table_name")

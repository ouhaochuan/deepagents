...
- [create_deep_agent](/libs\deepagents\deepagents\graph.py#L39-L160)：创建deepagent的主要函数
- [FilesystemMiddleware](/libs\deepagents\deepagents\middleware\filesystem.py#L781-L1084)：文件系统中间件
- [SubAgent](/libs\deepagents\deepagents\middleware\subagents.py#L16-L44) 和 [SubAgentMiddleware](/libs\deepagents\deepagents\middleware\subagents.py#L376-L483)：subagent相关类
- [CompiledSubAgent](/libs\deepagents\deepagents\middleware\subagents.py#L47-L57)：预编译subagent类

### 2. 主要创建函数 - [graph.py](/libs\deepagents\deepagents\graph.py)
这个文件包含了创建深度agent的核心逻辑 [create_deep_agent](/libs\deepagents\deepagents\graph.py#L39-L160) 函数：
- 集成了待办事项列表、文件系统和subagent功能
- 默认提供了文件操作工具(ls, read_file, write_file, edit_file, glob, grep, execute)
- 支持subagent功能，允许创建专门用途的subagent
- 内置了摘要和缓存中间件以优化性能
...
文件系统中间件：
- 为agent提供文件系统工具（ls, read_file, write_file, edit_file, glob, grep）
- 如果后端支持执行，还会提供 execute 工具
- 实现了工具结果过大时自动保存到文件系统的功能

#### [subagents.py](/libs\deepagents\deepagents\middleware\subagents.py)
subagent中间件：
- 提供 `task` 工具，允许创建临时subagent处理复杂任务
- 支持多种类型的subagent，每种都有专门的功能和提示
- 实现了subagent的生命周期管理
